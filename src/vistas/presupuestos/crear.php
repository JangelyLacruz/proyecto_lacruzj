    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content border-0">
            <div class="modal-header text-white" style="background: linear-gradient(135deg, #4e54c8, #8f94fb);">
                <h5 class="modal-title" id="crearPresupuestoModalLabel">
                    <i class="fas fa-file-invoice-dollar me-2"></i> Crear Nuevo Presupuesto
                </h5>
            ...
            <div class="modal-body">
                ...
                <form id="formPresupuesto" action="index.php?c=PresupuestoControlador&m=guardar_presupuesto" method="POST" novalidate>
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            <label for="numero_orden" class="form-label">N° Orden de Compra (Opcional)</label>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" id="id_iva" name="id_iva" required>
                    ...
                    <div class="card">
                        <div class="card-header bg-success text-white">
                            <strong>Resumen del Presupuesto</strong>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
            ...
            <div class="modal-footer" style="background-color: #f8f9fa;">
                ...
                <button type="submit" form="formPresupuesto" class="btn btn-primary" style="background: linear-gradient(135deg, #4e54c8, #8f94fb); border: none;">
            ...
            <select class="form-select item-select producto-select" name="detalles[INDEX][id_inv]" required>
                ...
                <?php foreach ($productos as $producto): ?>
                    <option value="<?= $producto['id_inv'] ?>" 
                            ...
                            data-costo-mayor="<?= $producto['costo_mayor'] ?? $producto['costo'] ?>"
                            data-unidad="<?= $producto['unidad_medida'] ?? 'Unidad' ?>"
                            data-stock="<?= $producto['stock'] ?>">
                        <?= htmlspecialchars($producto['nombre'] . ' - $' . $producto['costo'] . ' (' . ($producto['unidad_medida'] ?? 'Unidad') . ') - Stock: ' . $producto['stock']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <select class="form-select item-select servicio-select" name="detalles[INDEX][id_inv]" required>
                ...
                <?php foreach ($servicios as $servicio): ?>
                    <option value="<?= $servicio['id_inv'] ?>" 
                            ...
                            data-unidad="<?= $servicio['unidad_medida'] ?? 'Unidad' ?>">
                        <?= htmlspecialchars($servicio['nombre'] . ' - $' . $servicio['costo'] . ' (' . ($servicio['unidad_medida'] ?? 'Unidad') . ')') ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <div class="invalid-feedback">
