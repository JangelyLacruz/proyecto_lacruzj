window.cargarDetalleMaterias = function(idProducto, nombreProducto, esFabricado) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                if (response.data && response.data.materias_primas && response.data.materias_primas.length > 0) {
                    mostrarMateriasPrimasEnTabla(response.data.materias_primas);
                } else {
                    mostrarMensajeSinMaterias();
                }
            } else {
                mostrarErrorDetalle(response.message || 'Error al cargar los detalles del producto');
            }
        ...
        error: function(xhr, status, error) {
            ...
            if (xhr.responseText && xhr.responseText.trim().startsWith('<!DOCTYPE')) {
                mensajeError = 'Error del servidor: La respuesta no es válida. Contacte al administrador.';
            } else if (xhr.status === 404) {
                mensajeError = 'Función no encontrada en el servidor';
...
window.mostrarDetalles = window.cargarDetalleMaterias;
